...
UPPER_WHITE_HSV = np.array([190, 40, 100]) #115, 70, 100
...
def find_and_save_contours(mask_img, name):
    ...
    for contour in cnts:
        ...
        elements.append(Element(name, com, contour, width, height))
        #cv2.putText(image, shape, (cX, cY), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)


def locate_element(element_name, hsv_img):
    ...
    if element_name == "red_berr" or element_name == "wall":
        for filter_pair in element_color_filters:
            ...
            find_and_save_contours(mask, element_name)
...
def process_img(img):
    ...
    find_elements_on_img(opened_image)
